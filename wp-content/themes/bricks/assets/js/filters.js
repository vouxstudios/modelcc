		...
		if (filterMethod === 'ajax') {
			...
			const search = (e) => {
				...
				if (queryInstance?.isLiveSearch && searchValue === '') {
					...
					// Set isClearLiveSearch flag to true before making the API call
					queryInstance.isClearLiveSearch = true

					// Make API call to refresh filter options
					bricksUtils.fetchFilterResults(targetQueryId)

					// Reset flag after API completes (#86c3zpqyj)
					document.addEventListener(
						'bricks/ajax/query_result/displayed',
						function resetFlag(event) {
							if (event.detail.queryId === targetQueryId) {
								queryInstance.isClearLiveSearch = false
								document.removeEventListener('bricks/ajax/query_result/displayed', resetFlag)
							}
						}
					)
					return
		...
		if (filterMethod === 'ajax') {
			...
			element.addEventListener('click', function (e) {
				...
				if (queryInstance?.isLiveSearch) {
					...
					bricksUtils.hideLiveSearchWrapper(targetQueryId)
					// Set isClearLiveSearch flag to true before making the API call (#86c3zpqyj)
					queryInstance.isClearLiveSearch = true
...
function bricksLiveSearchWrappersInit() {
	...
	document.addEventListener('bricks/ajax/start', function (event) {
		...
		// Only show the live search wrapper if it's a live search and not a clear live search action (#(#86c3zpqyj)
		if (queryInstance?.isLiveSearch && !queryInstance?.isClearLiveSearch) {
